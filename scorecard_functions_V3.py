...
def SplitData(df, col, numOfSplit, special_attribute=[]):
    ...
    splitPoint = [rawValues[i] for i in splitPointIndex]  # zx：将splitPointIndex中的内容作为rawValues的index，以此进行分割.....
...
def BinBadRate(df, col, target, grantRateIndicator=0):
    ...
    total = pd.DataFrame({'total': total})  # zx: 给刚刚做出的series类型的total 加上列名：total
    bad = df.groupby([col])[target].sum()  # zx: 按照col的类别组合方式，对target进行汇总
    ...
    if N_distinct <= max_interval:  #如果原始属性的取值个数低于max_interval，不执行这段函数
        ...
        while (len(groupIntervals) > split_intervals):  # 终止条件: 当前分箱数＝预设的分箱数
            ...
            for k in range(len(groupIntervals)-1):
                ...
                df2b = regroup.loc[regroup['temp'].isin(temp_group)]  # zx: 将包含groupIntervals[k] + groupIntervals[k+1] 这两列的数据提取出来
        ...
        cutOffPoints = [max(i) for i in groupIntervals[:-1]]  # zx: [[3], [5], [8], [12], [16], [17], [18,19], [20], [21]]--->[[3], [5], [8], [12], [16], [17], [19], [20], [21]]
...
def BadRateEncoding(df, col, target):
    ...
    badRateEnconding = df[col].map(lambda x: br_dict[x])  # zx：或者仿照：dicts = dict(zip(regroup[col],regroup['bad_rate']))。将dataframe中的两列抽出，做成dict形式
...
def MergeBad0(df,col,target, direction='bad'):
    ...
    if direction == 'bad':
        # 如果是合并0坏样本率的组，则跟最小的非0坏样本率的组进行合
        regroup = regroup.sort_values(by='bad_rate')
    else:
        # 如果是合并0好样本率的组，则跟最小的非0好样本率的组进行合并
        regroup = regroup.sort_values(by='bad_rate',ascending=False)
    regroup.index = range(regroup.shape[0])  # zx:将regroup的index 赋值为0---shape[0]-1
    col_regroup = [[i] for i in regroup[col]]  # zx: 将col中对应的值，转换为----[[],[],[]]
    del_index = []
...
def Monotone_Merge(df, target, col):
    ...
    badRate = sorted(badrate_bin.items(), key=lambda x: x[0])  # zx:按照badrate_bin（dict）内的key值，进行升序排序
    badRate = [i[1] for i in badRate] # zx:取每一个list内第2个值并封装为list
...
def KS(df, score, target, plot = True):
    ...
    # print('all内容',all)
    # print('all列结构',all.dtypes)
    all[score] = all.index
    print(all)
    print(all.dtypes)
    print(all.columns)
    all = all.sort_values(by=score,ascending=False)  # by=['score'],axis=0
